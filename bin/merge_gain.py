...
Merging two species is only relabelling genomes into one clade, so NOTHING is
re-run: for a merged clade C the marker set is recomputed from counts.tsv alone,
    in_count_C  = sum of the members' per-species in_count
...
DEMOTE genes core to only one half (in_prev falls), so the net is not monotone and
has to be evaluated -- which is all this does, straight off the existing counts.

Greedy: at each step absorb the not-yet-merged same-genus species that shares the
most of the current clade's CORE genes. Stop as soon as the (capped) marker count
reaches --threshold -- the minimum requested marker count -- so the reported clade
is the SMALLEST merge that clears the bar (least resolution sacrificed). If the bar
is never reached, the peak-marker clade is reported instead.
...
where trajectory is the capped marker count after each absorbed species (so you can
see the rescue building), and merged_clade lists the seed + absorbed species.
...
too, so the gain reported here is a lower bound on the real one.
"""
import argparse
import os
import sys
...
def marker_set(clade_species, per, total, size, score, n_total, p):
    """(markers, core) cluster sets for the merged clade. Identical prevalence
    filter to SCORE, evaluated on the summed member counts."""
...
def probe_species(sp, genus, genus_species, per, total, size, score, n_total, p,
    ...
    cap = lambda n: min(n, p["max_per_clade"])
    clade = [sp]
    markers, core = marker_set(clade, per, total, size, score, n_total, p)
    steps = [(sp, cap(len(markers)))]          # (seed-or-added species, capped)
    for _ in range(max_steps):
        ...
        clade.append(nb)
        markers, core = marker_set(clade, per, total, size, score, n_total, p)
        steps.append((nb, cap(len(markers))))
...
def main():
    ...
    for sp in flagged:
        ...
        rows.append(probe_species(sp, genus, genus_species, per, total, size,
                                  score, n_total, p, args.threshold,
                                  args.max_steps))
